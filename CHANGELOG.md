...
### Added
- **Chained pre-commit hook adoption**: when `.githooks/pre-commit` is a foreign hook that already delegates into `.githooks/pre-commit.d/`, the installer no longer reports a conflict. It deploys the owned chain entry `.githooks/pre-commit.d/00-omp-reviewer-kit.chain` (execs the review runner) and repairs it on drift, leaving the foreign hook byte-identical. Foreign hooks without the chain marker still fail closed as `conflict`. Status reports `hookChained`/`chainedHookPresent`/`chainedHookCurrent` and the status command shows `OK (chained)`.
